...
public class FormularioInscricaoSAVBO extends AbstractBusinessObject<FormularioInscricaoSAV> {
    ...
    public void validate(FormularioInscricaoSAV formularioInscricaoSAV) throws BusinessException {
         ...
         CabosAutorizados cabo = cabosBO.cabosPelaMatricula(formularioInscricaoSAV.getMatricula());

        if (cabo == null) {
            throw new BusinessException("Servidor com a Matrícula ".concat(formularioInscricaoSAV.getMatricula()).concat(" não está autorizada a fazer esse concurso!"));
        }
        
        if (verificarDataInscricao(formularioInscricaoSAV.getDataInscricao())) {
            throw new BusinessException("Prazo de inscrição encerrado no dia 17/08/2018 as 23:59:59!");
        }
...
//        }
    ...
    private boolean verificarDataInscricao(Date dataInscricao) {
        ...
        dataFinal.set(Calendar.DAY_OF_MONTH, 17);
        dataFinal.set(Calendar.MONTH, 7);
