...
import br.gov.pi.ati.sisforms.dao.controleacesso.PerfilDAO;
    ...
    public void validate(Usuario usuario) throws BusinessException {
        if (usuario.isSuperUsuario()) {
            if (usuario.getOrgao() != null) {
                throw new BusinessException("Super Usuário não necessita de Órgão!");
            }
        } else {
            if (usuario.getOrgao() == null) {
                throw new BusinessException("Órgão é obrigatório para Usuáros que não são do tipo Super Usuário!");
            }
        }
    }
