...
public class PatrulhaBO extends AbstractBusinessObject<Patrulha> {
    ...
    public List<UniqueField> getUniqueFields() {
        ...
        uniqueFields.add(new UniqueField(Restriction.equals("ativo", true), "viatura").setMessage("Já existe uma guarnião ativa para viatura informada!"));
        return uniqueFields;
    }
    ...
    public void validate(Patrulha patrulha) throws BusinessException {
        ...
        if (guarnicoes.size() < 1) {
            throw new BusinessException("Uma Guarnião deve possuir um ou mais componentes!");
        } else {
            int contComandante = 0;
            int contMotorista = 0;

            for (GuarnicaoViatura guarnicao : guarnicoes) {
                if (guarnicao.getTipo() == TipoComponenteGuarnicao.COMANDANTE) {
                    contComandante++;
                }
                if (guarnicao.getTipo() == TipoComponenteGuarnicao.MOTORISTA) {
                    contMotorista++;
                }
            }

            if (contComandante < 1) {
                throw new BusinessException("Uma Guarnição deve possuir um comandante!");
            }

            if (contComandante > 1) {
                throw new BusinessException("Uma Guarnição deve possuir apenas um comandante!");
            }
        }
    }
