...
public class CadastroPessoalBO extends AbstractBusinessObject<CadastroPessoal> {
    ...
    public CadastroPessoal listarCadastro(String cpf) {
        ...
        cadastro = (CadastroPessoal) cadastroPessoalDAO.getQueryBuilder().select("cadastro").from(CadastroPessoal.class, "cadastro").innerJoinFetch("cadastro.carteiraDeTrabalho", "carteira")
                .innerJoinFetch("cadastro.rg", "rg").innerJoinFetch("cadastro.endereco", "endereco").innerJoinFetch("cadastro.naturalidade", "naturalidade")
                .leftJoinFetch("cadastro.titulo", "titulo").innerJoinFetch("cadastro.grauDeInstrucao", "grauDeInstrucao").leftJoinFetch("cadastro.formacaoProfissional", "formacaoProfissional")
                .add(restrictions).getSingleResult();
//        return cadastroPessoalDAO.unique("cpf", cpf != null ? cpf.replace(".", "").replace("-", "") : cpf);
        return cadastro;
