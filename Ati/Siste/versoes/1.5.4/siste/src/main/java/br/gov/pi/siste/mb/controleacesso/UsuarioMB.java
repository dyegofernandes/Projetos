...
public class UsuarioMB extends AbstractBaseBean<Usuario> implements Serializable {
    ...
    public void save() {
        ...
        if (getEntity().getOrgao() == null && getEntity().isSuperUsuario()) {
            try {
                //salvar usuario
            ...
            if (getEntity().getOrgao() == null && !getEntity().isSuperUsuario()) {
                FacesMessageUtils.error("Orgão é obrigatório para Usuários não administradores!");
            } else {
                if (getEntity().getUnidade() != null && getEntity().isSuperUsuario()) {
                    FacesMessageUtils.error("Super Usuários não necessitam de Orgão!");
                } else {
                    try {
                        //salvar usuario
                        usuarioBO.save(getEntity());
