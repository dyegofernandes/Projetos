...
public class ReferenciaBO extends AbstractBusinessObject<Referencia> {
                ...
                if (unidade.getOrgao().equals(orgao)) {
                    ...
                    for (Movimento movimento : movimentos) {
                        ...
                        if (movimento.getServidor().getDadosFuncionais().getUnidade().equals(unidade)) {
                            movimentoVO.setTipo(movimento.getTipo().toString());
    ...
    public String montarSefip(FiltrosVO filtros) {
        String corpo = "";
        filtros.setTipo(TipoPessoa.PF); //Nao pega pessoa juridica
        List<Movimento> movimentos = movimentoBO.listarMovimentos(filtros); //Usuario e Referencia    
    ...
    public List<ValorUsadoPorUnidadeVO> listarValorUsadoPorUnidadePF(FiltrosVO filtros) {
        List<ValorUsadoPorUnidadeVO> lista = new ArrayList<ValorUsadoPorUnidadeVO>();
        String queryString = "select u.codigo, u.nome, sum(valor) as valor_usado, u.valorlimite as valor_disponivel, cast ((sum(valor)*100/u.valorlimite) as numeric(19,2)) as porcentagem, m.tipo "
        ...
        if (filtros.getTipo() != null) {
            queryString = queryString.concat("AND m.tipo=MOV_TIPO").replace("MOV_TIPO", filtros.getTipo() == TipoPessoa.PF ? "0" : "1");
        } else {
            queryString = queryString.concat("AND m.tipo in (0,1) ");
        }

        if (filtros.getReferencia() != null) {
            queryString = queryString.concat("group by u.codigo, u.nome, u.valorlimite, m.tipo order by u.codigo, m.tipo;").
            ...
            for (Object[] tupla : resultado) {
                ...
                if (tupla[5] != null) {
                    if ((Integer) tupla[5] == 0) {
                        unidade.setTipo(TipoPessoa.PF.getDescricao());
                    } else {
                        unidade.setTipo(TipoPessoa.PJ.getDescricao());
                    }
                }
                lista.add(unidade);
            }
