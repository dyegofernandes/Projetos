...
public class MovimentoBO extends AbstractBusinessObject<Movimento> {
    ...
    @EJB
    private TabelaIRRFBO tabelaIrBO;
    ...
    public void validate(Movimento movimento) throws BusinessException {
        ...
        if (movimento.getValorBCalc() != null) {
    ...
    public void gerarRecibo(Movimento movimento) {
        ...
        valorLiquido = movimento.getValor().subtract(sestSenat).subtract(irrf).subtract(inss);
    ...
    public void clonarMovimento(Movimento movimento, Referencia referencia) {
        Movimento movClonado = new Movimento();
        ...
        movClonado.setBaseIss(movimento.getBaseIss());
        movClonado.setBaseSestSenat(movimento.getBaseSestSenat());
        movClonado.setCnpj(movimento.getCnpj());
        movClonado.setRazaoSocial(movimento.getRazaoSocial());
        movClonado.setDataInclusao(new Date());
//        movClonado.setDataPagamento(movimento.getDataPagamento());
        movClonado.setDesconto1(movimento.getDesconto1().compareTo(BigDecimal.ZERO) > 0
                ? calcularInss(movClonado.getValorBCalc()) : BigDecimal.ZERO);

        movClonado.setDesconto2(movimento.getDesconto2());
        movClonado.setFinanceiro(movimento.getFinanceiro());
        movClonado.setFonte(movimento.getFonte());

        movClonado.setIrrf(calcularDescontoPelaBaseIrrf(movClonado.getIr(), movClonado.getBaseIrrf(), movimento.getTipo()));
        movClonado.setIss(movimento.getIss());
//        movClonado.setNeMov(null);
//        movClonado.setNlMov(null);
        movClonado.setObservacao(movimento.getObservacao());
        movClonado.setOcorrencia(trabalhadorApareceNaCompetencia(referencia, movimento.getServidor()) ? Ocorrencia.MULTIPLOS_VINCULOS : null);
        movClonado.setPensao(movimento.getPensao());
        movClonado.setReferencia(referencia);
        movClonado.setServidor(movimento.getServidor());
        movClonado.setSestSenat(movimento.getSestSenat());
        movClonado.setTipo(movimento.getTipo());
        movClonado.setTravado(false);
        movClonado.setValor(movimento.getValor());

        movimentoDAO.saveOrMerge(movClonado, true);
    }

    public BigDecimal calcularIrrfEBase(Movimento movimento) {
        BigDecimal valor = BigDecimal.ZERO;
        if (movimento.getValor() != null) {
            valor = valor.add(movimento.getValor());
            if (movimento.getTipo() == TipoPessoa.PF) {
                if (movimento.getIr() != null) {
                    if (movimento.getPensao() != null) {
                        valor = valor.subtract(movimento.getPensao());
                    }

                    if (movimento.getDesconto1() != null) {
                        valor = valor.subtract(movimento.getDesconto1());
                    }
                    ...
                    if (servidor != null) {
                        ...
                        if (dados.getQtdDepIr() != null) {
                            valor = valor.subtract(movimento.getIr().getDeducaoPorDependentes().
    ...
    public BigDecimal calcularDescontoPelaBaseIrrf(TabelaIRRF tabelaIRRF, BigDecimal base, TipoPessoa tipo) {
        BigDecimal desconto = BigDecimal.ZERO;
        if (tabelaIRRF != null && base != null) {
            if (tipo == TipoPessoa.PF) {
                AliquotaIRRF aliquotaIrrfTemp = aliquotaIRRFBO.aliquotaPeloValor(tabelaIRRF, base);

                if (aliquotaIrrfTemp == null) {
                    return BigDecimal.ZERO;
                }

                BigDecimal aliquotaIrrf = aliquotaIrrfTemp.getValor();

                BigDecimal deducaoIrrf = aliquotaIrrfTemp.getParcelaADeduzir();

                if (deducaoIrrf != null && aliquotaIrrf != null) {
                    BigDecimal irrfTemp = util.calcularPorcentagem(aliquotaIrrf, base);
                    irrfTemp = irrfTemp.subtract(deducaoIrrf);
                    desconto = desconto.add(irrfTemp);
                }
            }

            if (tipo == TipoPessoa.PJ) {
                desconto = desconto.add(util.calcularPorcentagem(new BigDecimal(1.5), base));
            }
        }
        return desconto;
    }

    public BigDecimal calcularBaseInss(Movimento movimento) {
        BigDecimal base = BigDecimal.ZERO;
        if (movimento.getValor() != null) {
            base = base.add(movimento.getValor());
            if (movimento.getTipo() == TipoPessoa.PF) {
                if (movimento.getInss() != null) {
                    AliquotaInss aliquota = inssBO.pegarAliquota(movimento.getInss(), base);

                    if (aliquota.getValorFixo().compareTo(BigDecimal.ZERO) == 0) {
                        base = base;
                    } else {
                        base = aliquota.getSalarioInicial();
                    }
                }
            }
    ...
    public BigDecimal calcularInss(BigDecimal base) {
        BigDecimal desconto = BigDecimal.ZERO;

        if (base != null) {
            desconto = desconto.add(util.calcularPorcentagem(new BigDecimal(11), base));
        }
