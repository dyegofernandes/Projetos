package com.ebol.bo.controleacesso;
...
public class UsuarioBO extends AbstractBusinessObject<Usuario> {
    ...
    public void validate(Usuario usuario) throws BusinessException {
        if (usuario.isSuperUsuario() && usuario.getBanco() != null) {
            throw new BusinessException("Super usuário não necessita de uma Loja informada!!");
        } else {
            if (!usuario.isSuperUsuario() && usuario.getBanco() == null) {
