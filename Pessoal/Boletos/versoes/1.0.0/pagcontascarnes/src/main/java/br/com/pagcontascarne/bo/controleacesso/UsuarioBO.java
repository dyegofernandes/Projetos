package br.com.pagcontascarne.bo.controleacesso;
...
public class UsuarioBO extends AbstractBusinessObject<Usuario> {
    ...
    /**
     * nao pode-se repetir o email e login
     *
     * @return
     */
    @Override
    ...
    public List<Usuario> usuarios(FiltrosBusca filtros) {
        ...
        if (!Utils.isNullOrEmpty(filtros.getNome())) {
            restrictions.like("usuario.nome", filtros.getNome());
        }

        if (!Utils.isNullOrEmpty(filtros.getCpf_cnpj())) {
            restrictions.add("usuario.cpf", filtros.getCpf_cnpj());
        }

        if (!Utils.isNullOrEmpty(filtros.getEmail())) {
            restrictions.add("usuario.email", filtros.getEmail());
        }

        if (!Utils.isNullOrEmpty(filtros.getLogin())) {
            restrictions.like("usuario.userLogin", filtros.getLogin());
        }

        if (filtros.getAtivo() != null) {
            if (filtros.getAtivo() == Ativo.SIM) {
                restrictions.add("usuario.situacaoUsuario", SituacaoUsuario.ATIVO);
            } else {
                restrictions.add("usuario.situacaoUsuario", SituacaoUsuario.INATIVO);
            }
        }
