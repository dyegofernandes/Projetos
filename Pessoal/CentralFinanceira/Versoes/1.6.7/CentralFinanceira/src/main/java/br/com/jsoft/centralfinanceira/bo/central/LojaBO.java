...
public class LojaBO extends AbstractBusinessObject<Loja> {
    ...
    public List<DiaDiaVO> listaRecebimentoLoja(List<Loja> lojas, String periodo, String tipoRecebimento) {
        ...
        for (Object[] tupla : resultado) {
            ...
            BigDecimal previsaoQtd = previsao((BigDecimal) tupla[33], periodo);
            BigDecimal diferencaQtd = porcentagemDiferenca(previsaoQtd, (BigDecimal) tupla[66]);
            BigDecimal previsaoValor = previsao((BigDecimal) tupla[65], periodo);
            BigDecimal diferencaValor = porcentagemDiferenca(previsaoValor, (BigDecimal) tupla[67]);

            dia.setPrevisaoQtd(previsaoQtd);
            dia.setPrevisaoValor(previsaoValor);
            dia.setDiferencaQtd(diferencaQtd);
            dia.setDiferencaValor(diferencaValor);
    ...
    public BigDecimal previsao(BigDecimal total, String periodo) {
        ...
        if (total != null) {
            mediaDiaria = total.divide(diaUteis.compareTo(BigDecimal.ZERO) == 0 ? BigDecimal.ONE : diaUteis, 2, RoundingMode.HALF_UP);
        } else {
    ...
    public BigDecimal porcentagemDiferenca(BigDecimal previsao, BigDecimal totalAnterior) {
        if (previsao == null || totalAnterior == null) {
            return BigDecimal.ZERO;
        }
    ...
    public int diasUteis(boolean totalDiasMes, String periodo) {
        int workingDays = 0;
