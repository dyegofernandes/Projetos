...
public class VendaBO extends AbstractBusinessObject<Venda> {
    ...
    public String gerarTxtNota(Venda venda, String numNota, String numChaveAcesso, String complemento) {
...
//   ------------------ Fim Grupo B ----------------------
        ...
        c.setC17Ie(empresa.getInscricaoEstatual());
        c.setC19Im("");
        c.setC20Cnae("");
        ...
        if (venda.getCliente().getTipoIsencao() == TipoInsencaoCliente.CONTRIBUIENTE) {
            e.setE17Ie(venda.getCliente().getInscricaoEstatual());
        }
...
//            i.setI03Cean(prod.getProEan().replace("-", ""));  // nao sei o que eh
            ...
            i.setI09Ucom(item.getProduto().getUnidadeDeVenda().toString());
...
//            i.setI12Ceantrib(prod.getProEan().replace("-", ""));  //Nao sei o que eh
            i.setI13Utrib(item.getProduto().getUnidadeDeVenda().toString());
...
//            i.setI17Vdesc(formatarDouble(desconto).replace(",", ".")+"00");  //Nao sei o que eh
            ...
            //GRUPO N  
            n = gerarTributacao(item.getProduto().getCodigoCst().substring(1, 4), item.getProduto().getCodigoCst().substring(0, 1), item.getProduto().getCodigoCst().substring(1, 4), String.valueOf(item.getIcmsCompra()));
...
//        z.setZ03Infcpl("DADOS BANCARIOS: AG.: " + conta.getAgencia() + "CNTA CORRENTE: " + conta.getConta() + ", " + conta.getBanco().getNome() + "-" + conta.getBanco().getSigla());
        z.setZ03Infcpl(complemento != null ? complemento : "");
