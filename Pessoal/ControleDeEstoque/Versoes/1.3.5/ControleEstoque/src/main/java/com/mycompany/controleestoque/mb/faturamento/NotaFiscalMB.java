...
public class NotaFiscalMB extends AbstractBaseBean<NotaFiscal> implements Serializable {
    ...
    public void gerarArquivoTextNfe(NotaFiscal nota) throws IOException, InterruptedException {
        ...
        String texto = vendaBO.gerarTxtNota(venda, nota.getNumeroNota().toString(), nota.getNumChaveDeAcesso(), nota.getCampoComplementar());
        Thread.sleep(5000);
        FacesUtils.download(texto.getBytes(), "text/plain", "ArquivoTextoNE" + nota.getId() + ".txt");
    }
    ...
    private String pegaUltimoComplemento() {
        if (getEntity().getVenda() != null) {
            return notaFiscalBO.pegarUltimoComplemento(getEntity().getVenda());
        }
        return null;
    }
    ...
    public void carregarValores() {
        ...
        getEntity().setCampoComplementar(pegaUltimoComplemento());
    }
