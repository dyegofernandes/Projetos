...
//            Exemplo NFe22150619843574000150550010000001631000009480
    ...
    public void visualizarNFe(NotaFiscal nota) {
        ...
        FacesJasper.createJasperReport(listaVO, params,
                "/WEB-INF/report/relatorios/NFeModelo1.jasper", "NFe - ".concat(cliente.getRazaoSocial()).concat(".pdf"));
    }
    ...
    public List<NotaFiscal> notasPorLote(LoteEnvio lote) {

        restrictions = new Restrictions();

        List<NotaFiscal> lista = new ArrayList<NotaFiscal>();

        restrictions.add("loteEnvio", lote);
        
        lista = notaFiscalDAO.list(restrictions);

        return lista;
    }
    
    public List<NotaFiscal> notasAbertas() {

        restrictions = new Restrictions();

        List<NotaFiscal> lista = new ArrayList<NotaFiscal>();

        restrictions.add("status", StatusNF.ACONFIRMAR);
