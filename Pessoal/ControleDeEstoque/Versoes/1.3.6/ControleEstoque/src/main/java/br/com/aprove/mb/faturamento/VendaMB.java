...
public class VendaMB extends AbstractBaseBean<Venda> implements Serializable {
    ...
    public void init() {
        ...
        if (getEntity().getId() != null) {
            itensVenda = itemDAO.list("venda_id", getEntity().getId());
        } else {
    ...
    public void save() {
        ...
        if (seTiverId && getEntity().isAtivo()) {
            for (ItemVenda item : itensVenda) {                
                Estoque estoque = getBO().getDAO().getInitialized(item.getEstoque());
                
                Produto produto = getBO().getDAO().getInitialized(estoque.getProduto());

                for (ItemVenda itemTemp : this.itensVenda) {

                    if (itemTemp.getId().equals(item.getId())) {
                        BigDecimal qtdNova = BigDecimal.ZERO;
...
//            }
    ...
    private boolean constaisItem(ItemVenda item) {
        for (ItemVenda itemTemp : itensVenda) {
            if (itemTemp.getId().equals(item.getId())) {
                return true;
            }
        }
