        ...
        for (Iterator<String> i = getServices().keySet().iterator(); i.hasNext();) {
            ...
            for (Object params : getServices().get(serviceName).getParameters()) {
                if (params instanceof LinkedHashMap) {
                    LinkedHashMap<String, Object> lhm = (LinkedHashMap<String, Object>) params;
...
* @param writer Para onde escreveremos
    ...
    public void writeContent(Writer writer) throws IOException {
        writer.write("Transport: " + this.getTransport());
        writer.write('\n');
        writer.write("Envelope: " + this.getEnvelope());
        writer.write('\n');
        writer.write("ContentType: " + this.getContentType());
        writer.write('\n');
        writer.write("Target: " + this.getTarget());
        writer.write('\n');
        ...
        for (Iterator<String> i = getServices().keySet().iterator(); i.hasNext();) {
            ...
            for (Object params : getServices().get(serviceName).getParameters()) {
                if (params instanceof LinkedHashMap) {
                    ...
                    for (Iterator<String> j = lhm.keySet().iterator(); j.hasNext();) {
                        String paramKey = j.next();
                        writer.write("\t\t\t" + paramKey + ": " + lhm.get(paramKey));
                        writer.write('\n');
                    }
                    writer.write('\n');
                }
